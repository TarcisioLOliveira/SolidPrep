...
void SolverManager::generate_matrix(const Meshing* const mesh, const std::vector<double>& density, double pc, double psi){
    ...
    size_t l_num = 0;
    const size_t l_num_orig = mesh->lag_node_map.size();
    ...
    for(size_t i = 0; i < mesh->sub_problems->size(); ++i){
        if(this->iteration == 0){
            if(needs_lambda){
                ...
                std::fill(this->lambdas[i][0].begin(), this->lambdas[i][0].begin() + l_num_orig, 0);
            } else {
...
void SolverManager::calculate_displacements_adjoint(const Meshing* const mesh, std::vector<std::vector<double>>& load, std::vector<double>& u){
    ...
    for(size_t i = 0; i < mesh->sub_problems->size(); ++i){
        ...
        if(l.size() == mesh->max_dofs){
            ...
            this->solvers[i]->calculate_displacements(mesh, load_pos, ui, this->lambdas[i][this->solve_step]);
            ...
            this->solvers[i]->calculate_displacements(mesh, l, ui, this->lambdas[i][this->solve_step]);
