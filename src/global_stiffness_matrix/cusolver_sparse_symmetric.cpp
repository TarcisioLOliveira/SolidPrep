...
cuSolverSparseSymmetric::cuSolverSparseSymmetric(){
    cudaSetDevice(0);
}

void cuSolverSparseSymmetric::generate(const Meshing* const mesh, const std::vector<double>& density, const double pc, const double psi){
    ...
    this->n = mesh->load_vector.size();
    this->sK.generate_csr(n);
    this->nnz = this->sK.nvals;
    if(this->first_time){
        this->first_time = false;
        this->csrRowPtr.allocate(this->sK.rows);
        this->csrColInd.allocate(this->sK.cols);
        this->csrVal.allocate(this->sK.vals);
    } else {
        this->csrVal.set_data(this->sK.vals);
    }
    logger::quick_log("Done.");
}

void cuSolverSparseSymmetric::add_geometry(const Meshing* const mesh, const Geometry* const g){
    ...
    for(auto& e : g->mesh){
        ...
        this->sK.insert_matrix_symmetric(k, u_pos);
    }
}

void cuSolverSparseSymmetric::add_geometry(const Meshing* const mesh, const Geometry* const g, std::vector<double>::const_iterator& rho, const double pc, const double psi){
    const double t = mesh->thickness;
    const size_t dof      = mesh->elem_info->get_dof_per_node();
    const size_t node_num = mesh->elem_info->get_nodes_per_element();
