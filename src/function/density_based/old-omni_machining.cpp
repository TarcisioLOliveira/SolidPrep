...
#include <numeric>
#include <set>
#include "function/density_based/radial_machining.hpp"
#include "logger.hpp"
...
RadialMachining::RadialMachining(const Meshing* const mesh, const DensityFilter* const filter, gp_Pnt center, gp_Dir axis, double v_norm, double beta)
    : mesh(mesh), filter(filter), center(center), axis(axis), v_norm(v_norm), beta(beta), proj(Projection::Parameter{64, 64, 0, 0}, 0.5){}
...
double RadialMachining::calculate_with_gradient(const Optimizer* const op, const std::vector<double>& u, const std::vector<double>& x, std::vector<double>& grad){
    ...
    for(const auto& g:mesh->geometries){
        if(g->do_topopt){
            ...
            for(const auto& e:g->mesh){
                ...
                bool boundary = false;
                const auto phi_e = e->get_phi_radial(this->mesh->thickness, this->beta, this->v_norm, v, dv, this->heaviside(*x_it, BETA_RHO, 0.5));
                for(size_t i = 0; i < num_nodes; ++i){
                    const auto& ni = e->nodes[i];
                    const long id1 = this->id_mapping[ni->id];
                    if(id1 < 0){
                ...
                if(boundary){
                    for(size_t i = 0; i < num_nodes; ++i){
                        ...
                        for(size_t j = 0; j < num_nodes; ++j){
                            ...
                            const long id2 = this->id_mapping[nj->id];
                            if(id2 < 0){
                                this->b[id1] -= phi_e[i*num_nodes + j];
                            }
                        }
                    }
                }
                ...
                *d_it = 1;
                ...
                for(size_t i = 0; i < num_nodes; ++i){
                    ...
                    if(id1 < 0){
                        ...
                        psi_e += psi[id1]/num_nodes;
                    }
                }
                *d_it -= this->heaviside(psi_e, BETA_PSI, 0.5);
                *d_it *= 1.0 - *x_it;
                ++d_it;
                x_it += num_den;
            }
        ...
        if(g->do_topopt){
            ...
            for(const auto& e:g->mesh){
                ...
                for(size_t i = 0; i < num_nodes; ++i){
                    ...
                    b_grad[id1] -= (*dg_it)*this->heaviside_grad(psi_e, BETA_PSI, 0.5)*(1.0 - *x_it)/num_nodes;
                }
                ++dg_it;
                x_it += num_den;
            }
    ...
    for(const auto& g:mesh->geometries){
        if(g->do_topopt){
            ...
            //     i *= this->beta;
            // }
            double dHx = this->heaviside_grad(*x_it, BETA_RHO, 0.5);
            for(auto& p:phi_e){
                p *= dHx;
            }
            for(const auto& e:g->mesh){
                ...
                *g_it -= this->heaviside(psi_e, BETA_PSI, 0.5);
                *g_it *= -(*hg_it);
                ...
                if(boundary){
                    for(size_t i = 0; i < num_nodes; ++i){
                        ...
                        for(size_t j = 0; j < num_nodes; ++j){
                            ...
                            const long id2 = this->id_mapping[nj->id];
                            if(id2 < 0){
                                *g_it -= psi_tilde[id1]*phi_e[i*num_nodes + j];
                            }
                        }
                    }
                }
