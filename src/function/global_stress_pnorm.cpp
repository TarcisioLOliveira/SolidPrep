...
GlobalStressPnorm::GlobalStressPnorm(const Meshing* const mesh, FiniteElement* fem, double pc, double P, double pt, double psiK, double psiS):
...
double GlobalStressPnorm::calculate_with_gradient(const Optimizer* const op, const std::vector<double>& u, const std::vector<double>& x, std::vector<double>& grad){
    ...
    auto x_it0 = x.begin();
    for(const auto& g:this->mesh->geometries){
        g->get_stresses(u, 0.0, this->K_MIN, this->psiS, x_it0, s_it);
    }
                ...
                if(num_mat == 1){
                    ...
                    for(const auto& e:g->mesh){
                        ...
                        for(size_t i = 0; i < num_den; ++i){
                            ...
                            const auto c = e->get_centroid();
                            double S = *stress_it;
                            double Se = e->von_Mises_derivative(D_S, gradD_S[i], v*std::pow(S, P-2), c, u);
