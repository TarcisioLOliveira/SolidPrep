...
TopoDS_Shape StandardSizing::boundary_expansion_approach(){
    ...
    this->solver->generate_matrix(&mesh, mesh.load_vector[0].size(), 0, mesh.node_positions[0], false, std::vector<std::vector<double>>(), FiniteElement::ProblemType::RIGID);
    std::vector<double> u(mesh.load_vector[0]);
    std::vector<double> lambda;
    this->solver->calculate_displacements(&mesh, u, lambda);
