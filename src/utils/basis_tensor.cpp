...
std::vector<double> basis_tensor_3D(const Eigen::Matrix<double, 3, 3>& R){
    ...
    std::array<double, 9> K4 =
        {R(0,0)*R(1,1)+R(0,1)*R(1,0), R(2,0)*R(0,1)+R(2,1)*R(0,0), R(1,0)*R(2,1)+R(1,1)*R(2,0),
         R(0,2)*R(1,0)+R(0,0)*R(1,2), R(2,2)*R(0,0)+R(2,0)*R(0,2), R(1,2)*R(2,0)+R(1,0)*R(2,2),
         R(0,1)*R(1,2)+R(0,2)*R(1,1), R(2,1)*R(0,2)+R(2,2)*R(0,1), R(1,1)*R(2,2)+R(1,2)*R(2,1)};
