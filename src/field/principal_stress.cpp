...
void PrincipalStress::generate(){
    ...
    this->base_D.reserve(proj_geoms.size());
    for(auto& g:proj_geoms){
        ...
        for(auto& m:mats){
            if(m->get_field() == this){
                ...
                if(DIM == 2){
                    base_D.push_back(m->stiffness_2D(nullptr, gp_Pnt(0,0,0)));
                } else if(DIM == 3){
                    base_D.push_back(m->stiffness_3D(nullptr, gp_Pnt(0,0,0)));
                }
                for(auto& e:g->mesh){
                    this->elem_mapping[e.get()] = &base_D.back();
                }
                break;
            }
        }
...
math::Matrix PrincipalStress::get_matrix(const MeshElement* e, const gp_Pnt& p) const{
    if(this->first_run){
        ...
        auto tensor = e->get_stress_tensor(*this->elem_mapping.at(e), p, this->u);
        math::Eigen eigen(tensor);
        logger::log_assert(std::abs(eigen.eigenvectors.determinant()) > 1e-7,
                logger::ERROR,
                "eigen vector singular");
        logger::log_assert(
                (eigen.eigenvectors*eigen.eigenvectors.T()).is_equal(math::Matrix::identity(3)),
                logger::ERROR,
                "eigen vector not orthonormal");
        std::vector<double> abs_sort(3);
        for(size_t i = 0; i < 3; ++i){
            abs_sort[i] = std::abs(eigen.eigenvalues[i]);
        }
        std::sort(abs_sort.begin(), abs_sort.end(), std::greater_equal<double>());
